---
title: FastAPI
---

# [FastAPI](https://fastapi.tiangolo.com/)

## Links

- [FastAPI and cooperative multi-threading (2022)](https://aivarsk.com/2022/01/21/fastapi-concurrency/)
- [The FastAPI Ultimate Tutorial (2021)](https://christophergs.com/python/2021/12/04/fastapi-ultimate-tutorial/)
- [Concurrency with FastAPI](https://medium.com/cuddle-ai/concurrency-with-fastapi-1bd809916130)
- [TDD Approach to Create an Authentication System With FastAPI Part 3](https://santoshk.dev/posts/2021/tdd-approach-to-create-an-authentication-system-with-fastapi-part-3/)
- [Async Architecture with FastAPI, Celery, and RabbitMQ](https://medium.com/cuddle-ai/async-architecture-with-fastapi-celery-and-rabbitmq-c7d029030377)
- [FastAPI Best Practices (2022)](https://github.com/zhanymkanov/fastapi-best-practices)

## Packages
- [Build API with ASGI in AWS Lambda with API Gateway HTTP API or REST API, or with Function URL ✨](https://github.com/yezz123/asgi-aws)
- [FastAPI + Postgres project generator](https://github.com/lorenzocesconetto/fastapi-postgresql)
